...
public abstract class MagicItem implements Tradeable, MagicEffectRealization, MagicSource {
	...
	public MagicItem(String name, int usages, int price, int weight) {
        ...
        if (usages < 0 || price < 0 || weight < 0) throw new IllegalArgumentException();
        this.name = name;
        this.usages = usages;
        this.price = price;
        this.weight = weight;
	...
	public String toString() {
		...
		return String.format("[%s; %d g; %d %s; %d %s%s]",
		        name, weight, price, currency, usages, usageString(), additionalOutputString());
	}
