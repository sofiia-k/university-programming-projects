...
public class Wizard implements MagicSource, Trader, MagicEffectRealization {
	...
	public Wizard(String name, MagicLevel level, int basicHP, int HP, int basicMP, int MP, int money,
		...
		if (basicHP < 0 || HP < 0 || MP < 0 || money < 0 || carryingCapacity < 0) throw new IllegalArgumentException();
		if (basicMP < level.toMana()) throw new IllegalArgumentException();
		if (knownSpells == null || protectedFrom == null || inventory == null) throw new IllegalArgumentException();
		
		int totalWeight = 0;
		for (Tradeable t : inventory) {
		    totalWeight += t.getWeight();
		}
		if (totalWeight > carryingCapacity) throw new IllegalArgumentException();
		
		this.name = name;
		this.level = level;
		this.basicHP = basicHP;
		this.HP = (HP < 0) ? basicHP : HP;
		this.basicMP = basicMP;
		this.MP = (MP < 0) ? basicMP : MP;
		this.money = money;

		this.knownSpells = new java.util.HashSet<>(knownSpells);
		this.protectedFrom = new java.util.HashSet<>(protectedFrom);
	...
	public boolean castRandomSpell(MagicEffectRealization target) {
	    ...
	    return castSpell(chosen, target);
	}
	...
	public boolean useRandomItem(MagicEffectRealization target) {
		...
		Tradeable chosen = null;
	    int i = 0;
	    for (Tradeable m : inventory) {
	        if (i == index) {
	            chosen = m;
	            break;
	        }
	...
	public boolean steal(Trader thief) {
	    ...
	    if (!thief.hasCapacity(chosen.getWeight())) {
	    	this.removeFromInventory(chosen);
	    	return false;
	    }
	    
	    boolean added = thief.addToInventory(chosen);
	    if (added) {
	        this.removeFromInventory(chosen);
	        return true;
	    } else {
	...
	public boolean loot(Trader looter) {
	    ...
	    if (this.isLootable()) {
	        boolean successfulLoot = false;
	        while (!this.inventory.isEmpty()) {
	            if (this.steal(looter)) {
	                successfulLoot = true;
	            }
	        }
	        return successfulLoot;
	    }
	    return false; 
	}
